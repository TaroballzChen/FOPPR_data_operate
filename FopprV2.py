...
def result_data_in_xlsx(filename='resultdata.xlsx'):
    ...
    for origin_filename in All_of_file:
        ...
        if len(nomalize_signal_result[j]) == len(total_logconc_value[j]):
            for i in range(len(nomalize_signal_result[j])):
                result_data_worksheet.write_number('A%d'%(i+1),float(total_logconc_value[j][i]))
                result_data_worksheet.write_number('B%d'%(i+1),float(nomalize_signal_result[j][i]))
            else:
                chart = result_data_workbook.add_chart({'type':'scatter'})
                chart.add_series({
                    'name':'Sample',
                    'categories':'=%s'%origin_filename+'!A1:A%d'%(i+1),
            ...
            print("normalize signal & log concentration doesn't match with the list is out of range")
        j+=1
    else:
